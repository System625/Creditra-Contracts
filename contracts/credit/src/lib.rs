...
impl Credit {
    ...
    /// Reverts if credit line does not exist, is Closed, is not Active, borrower has not authorized,
...
mod test {
    ...
    fn test_repay_credit_succeeds_when_suspended() {
        ...
        let credit_line = client.get_credit_line(&borrower).unwrap();
        assert_eq!(credit_line.status, CreditStatus::Suspended);
        assert_eq!(credit_line.utilized_amount, 200_i128);
    }
...
mod test_close_utilized {
    ...
    fn test_close_utilized_borrower_rejected_on_suspended_line() {
        ...
        client.init(&admin);
        client.open_credit_line(&borrower, &1000_i128, &300_u32, &70_u32);
        // Draw while Active (draw_credit requires Active status).
        client.draw_credit(&borrower, &200_i128);
