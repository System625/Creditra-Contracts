...
/// Instance storage key for protocol token.
fn protocol_token_key(env: &Env) -> Symbol {
...
fn require_protocol_token(env: &Env) -> Address {
    env.storage()
        .instance()
        .get(&protocol_token_key(env))
...
impl Credit {
    ...
    pub fn set_protocol_token(env: Env, token: Address) -> () {
        ...
        env.storage()
            .instance()
            .set(&protocol_token_key(&env), &token);
        ()
    }
    ...
    /// Reverts if credit line does not exist, is Closed, or borrower has not authorized.
    /// Accepts protocol token repayment via allowance + transfer_from.
    ...
    pub fn repay_credit(env: Env, borrower: Address, amount: i128) -> () {
        ...
        let token = require_protocol_token(&env);
...
mod test {
    ...
    fn setup_protocol_token(
        ...
        token_admin_client.mint(borrower, &mint_amount);

        client.set_protocol_token(&token_id);

        let token_client = TokenClient::new(env, &token_id);
        let expiration_ledger = 1_000_u32;
        token_client.approve(borrower, contract_id, &allowance, &expiration_ledger);
    ...
    fn test_repay_credit_transfers_token_and_consumes_allowance() {
        ...
        let token_id =
            setup_protocol_token(&env, &admin, &borrower, &contract_id, &client, 1_000, 600);
    ...
    fn test_repay_credit_reverts_on_insufficient_allowance() {
        ...
        setup_protocol_token(&env, &admin, &borrower, &contract_id, &client, 1_000, 100);
    ...
    fn test_repay_credit_reduces_utilized_and_emits_event() {
        ...
        setup_protocol_token(&env, &admin, &borrower, &contract_id, &client, 1_000, 200);
    ...
    fn test_repay_credit_saturates_at_zero() {
        ...
        client.open_credit_line(&borrower, &1000_i128, &300_u32, &70_u32);
        client.draw_credit(&borrower, &100_i128);
        setup_protocol_token(&env, &admin, &borrower, &contract_id, &client, 1_000, 500);
